...
def match_param(DF,DEFINITIONS):
    ...
    for col in DF.columns:
        ...
        if len(col) > 2:
            ...
            match = submatch[submatch['unit'].str.contains(col[2], regex=False, case=False, na=False, regex=False, case=False, na=False)]
            ...
            if "Unnamed" not in col[1]:  #check for a null value in the units column
                match = submatch[submatch['unit'].str.contains(col[1], regex=False, case=False, na=False)]
                #print ("our match: ", match)
            else:
            ...
            #print ("col length  == 1")
            #print (col, "our submatch: ", submatch)
            match = submatch[submatch['unit'].str.contains(col[0], regex=False, case=False, na=False)]
